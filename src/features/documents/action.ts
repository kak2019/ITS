    ...
    async (oldItems: IFile[]) => {
        ...
        while (hasNext) {
            try {
                ...
                newItems = newItems.concat(response.map((item: IResponseItem) => {
                    return {
                        Id: item.Id,
                ...
                hasNext = false;
                Logger.write(`${CONST.LOG_SOURCE} (_readAllFilesSize) - ${JSON.stringify(err)}`, LogLevel.Error);
        ...
        try {
            ...
            for (let i = 0; i < ids.length; i++) {
                ...
                batchedSP.web.lists
                    .getByTitle(CONST.LIBRARY_NAME)
                    .items
                    .getById(ids[i]).update({ Title: `${item.FileLeafRef}-Updated` })
                    .then(r => res.push(r))
            ...
            Logger.write(`${CONST.LOG_SOURCE} (_updateTitles) - ${JSON.stringify(err)}`, LogLevel.Error);
    ...
    async (oldItems: IFile[]) => {
        ...
        while (hasNext) {
            try {
                ...
                hasNext = false;
                Logger.write(`${CONST.LOG_SOURCE} (_getDemoItems) - ${JSON.stringify(err)}`, LogLevel.Error);
                return Promise.reject(MESSAGE.retrieveDataFailed);
            }
        }
        const uniqueItems = Array.from(new Set(newItems.map(item => item.Id)))
