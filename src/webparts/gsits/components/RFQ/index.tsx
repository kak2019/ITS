...
const RFQ: React.FC = () => {
  ...
  const [rfqReleaseDateTo, setRfqReleaseDateTo] = useState<Date | undefined>(
    undefined
  );
  const [rfqReleaseDateFrom, setRfqReleaseDateFrom] = useState<
    Date | undefined
  >(undefined);
  const [rfqDueDateFrom, setRfqDueDateFrom] = useState<Date | undefined>(
    undefined
  );
    ...
    const fetchData = async (): Promise<void> => {
      try {
        ...
        const response = await client.get(
          `${CONST.azureFunctionBaseUrl}/api/GetParma?q=981`,
      ...
      {isSearchVisibel && (
        ...
        >
          ...
          <Dropdown
            ...
            onChange={(e, option) => setRequestType(String(option?.key || ""))}
            options={typeOptions}
            styles={fieldStyles}
          />
          <TextField
            label="RFQ No."
            value={rfqNumber}
            onChange={(e, newValue) => setRFQNumber(newValue || "")}
            styles={fieldStyles}
          />
          {fieldWithTooltip(
            t("Buyer"),
            ...
            <TextField
              value={buyer}
              onChange={(e, newValue) => setBuyer(newValue || "")}
              styles={fieldStyles}
            />
          )}
          {fieldWithTooltip(
          ...
          <Dropdown
            label="Status"
            selectedKey={status}
            onChange={(e, option) => setStatus(String(option?.key || ""))}
          ...
          <TextField
            label="Parma"
            value={parma}
            onChange={(e, newValue) => setParma(newValue || "")}
            styles={fieldStyles}
          />
          <DatePicker
